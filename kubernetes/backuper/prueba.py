...
def create_backup_as_tarfile(dump_filename: str) -> str:
    """
    ...
    try:
        with tarfile.open(tarfile_name, "w:gz") as tar:
            ...
            tar.add(process.communicate()[0])
            tar.close()
        log.info(f'File {dump_filename} successfully compressed to {tar_filename} with timestamp {datetime.now()}.')
        os.remove(dump_filename)
        log.info(f'File {dump_filename} successfully deleted.')
        return tar_filename
    except Exception as ex:
        log.error(f'Error while trying to compress {dump_filename}. Error {ex}')
...
tarfile_name: str = create_backup_as_tarfile('sip-' + str(datetime.now()) + '-dump.sql')
