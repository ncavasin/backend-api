...
def make_tarfile_from_dump(dump_filename: str) -> str:
    """
    ...
    """
    tar_filename: str = 'backup/' + dump_filename
    try:
        with tarfile.open(os, "w:gz") as tar:
            tar.add(tar_filename)
        tar.close()
        log.info(
            f'File {dump_filename} successfully compressed to {tar_filename} with timestamp {datetime.now()}.')
...
# fallback filename for testing
dump_name: str = 'myfile.txt'
# fallback filename for testing
tar_name: str = 'myfile.txt'
log.basicConfig(level=log.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

log.info('hi from python backuper!')
#dump_name = generate_dump()
#tar_name = make_tarfile_from_dump(dump_name)
upload_tarfile(tar_name, ServiceAccountCredentials.from_json_keyfile_dict(CREDENTIALS))
